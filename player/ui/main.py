...
class UIConstants:
    ...
    COVER_ART_SIZE = 48
    SIDEBAR_WIDTH = 300
    PROGRESS_BAR_MIN_WIDTH = 200  # Note: Increased from 150 for better usability
    TITLE_MIN_WIDTH = 50
    TITLE_MAX_WIDTH = 250  # Note: Increased for better readability
...
class MusicApp(Adw.Application):
    def __init__(self, **kwargs):
        ...
        self.current_theme = self._load_theme_preference()  # Note: Load saved theme or default
        
        # Note: Register actions
        self.create_actions()
    
    def create_actions(self):
        """Create application-level actions for menu."""
        # Note: Setup wizard action
    ...
    def on_upload_music(self, action, param):
        ...
        if hasattr(self, 'win'):
            ...
            if not config:
                # Note: Show dialog to prompt setup
    ...
    def on_activate(self, app):
        ...
        # Note: Load and apply saved theme preference
    ...
    def _load_theme_preference(self):
        ...
        try:
            ...
            import traceback
            traceback.print_exc()
        return 'default'  # Note: Default to system theme
    ...
    def _save_theme_preference(self, theme_name):
        ...
        try:
            ...
            # Note: Load existing preferences or create new
            prefs = {}
            if prefs_file.exists():
                with open(prefs_file, 'r') as f:
                    prefs = json.load(f)
            
            # Note: Update theme preference
    ...
    def set_theme(self, theme_name):
        ...
        self.current_theme = theme_name  # Note: Track current theme
        style_manager = Adw.StyleManager.get_default()
        display = Gdk.Display.get_default()
        
        # Note: Defensive check ensure display is valid
        if display is None:
            print(f"ERROR: Display is None! Cannot set theme. This shouldn't happen.")
            return
        

        # Note: Remove existing theme provider if any
        ...
        # Note: Apply theme based on selection
        try:
            if theme_name == "light":
                style_manager.set_color_scheme(Adw.ColorScheme.FORCE_LIGHT)
                # Note: Load light theme CSS
            ...
            else:  # Note: "Default" or any other value
...
class MainWindow(Adw.ApplicationWindow):
    def __init__(self, *args, **kwargs):
        ...
        # Note: Application icon from repo branding (ui_web/assets/icons generated from branding/)
        ...
        self.engine.set_track_load_callback(self._on_queue_next_item)  # Note: For auto-play from queue
        
        
        # Note: MPRIS integration (optional - requires mpris_server package)
        try:
        ...
        # Note: View stack for switching between main and mini modes
        ...
        # Note: Settings menu button (right side)
        ...
        # Note: Playback controls container (bottom bar)
        # Note: We need to restructure this to have [cover] [title/artist] [controls] [volume]
        # Note: Current structure seems to be just buttons. let's inspect where this is added.
        
        # Note: The view above shows `player_container` having play button and progress bar.
        # Note: But earlier I thought it was `playback_box`.
        
        # Note: Let's look at lines 150-174 in the view.
        # Note: It adds `play_btn` and `progress_scale` to `player_container`.
        # Note: This seems to be a reliable place.
        
        # Note: Revamped layout details
        # Note: [Image] [col title, artist] [space] [prev] [play] [next] [space] [progress]
        
        # Note: Let's rebuild the container content carefully.
        
        # Note: Player control bar (top)
        ...
        self.cover_art.set_visible(False)  # Note: Hide initially when no music playing
        
        # Note: Make cover art clickable
        ...
        # Note: Scrolledwindow for title (marquee)
        ...
        # Note: No ellipsize, allow full width for scrolling
        ...
        # Note: 3. Progress bar (scale) - moved left
        ...
        self.progress_scale.set_range(0, 100) # Note: Default, updated on play
        self.progress_scale.set_hexpand(True) # Note: Fill remaining space
        ...
        # Note: 4. Playback controls - moved right
        
        # Note: Previous details
        ...
        # Note: Content area (split view)
        # Note: Check for config and show wizard if needed
        config = self._load_config()
        if config:
            self._init_player_ui(self.main_view, config)
        else:
            # Note: No config - show setup wizard
            self._show_first_run_wizard()
        
        # Note: Initialize mini player view
        self._init_mini_player_ui()
        
        # Note: Connect time update
        self.engine.set_time_update_callback(self.on_time_update)
        self._seeking = False
        
        # Note: Poll engine state to update button icon (simple fix)
        GLib.timeout_add(250, self._poll_engine_state)
        
        
        # Note: Keyboard shortcuts for playback control
        # Note: Use capture phase to intercept keys before child widgets
    ...
    def _on_key_pressed(self, controller, keyval, keycode, state):
        ...
        # Note: Don't intercept keys when typing in text entry widgets
        focus_widget = self.get_focus()
        if isinstance(focus_widget, (Gtk.Entry, Gtk.SearchEntry, Gtk.Text, Gtk.TextView)):
            return False  # Note: Let text input work normally
        
        # Note: Space play/pause toggle
        if keyval == Gdk.KEY_space:
            self.on_play_toggle(None)
            return True
        
        # Note: Left arrow seek backward 10 seconds
        ...
        # Note: Right arrow seek forward 10 seconds
    ...
    def _init_mini_player_ui(self):
        ...
        self.mini_title.add_css_class("title-2") # Note: Large title
    ...
    def on_track_ended(self):
        ...
        # Note: Stop engine to reset state and update UI (run on main thread)
        def _stop_safe():
            self.engine.stop()
            return False
    ...
    def on_play_toggle(self, button):
        if self.engine.is_playing:
            ...
            if self.engine.current_track:
                 self.engine.pause() # Note: Resume
            else:
                 # Note: Try to play the first track if available
    ...
    def on_seek(self, scale):
        # Note: Determine if this value change is from user or automated
        # Note: Simplest way check if generic 'state' flags (GTK3 had event check, GTK4 assumes interaction if focus?)
        # Note: For simplicity, we might just seek.
        # Note: But to prevent loops, we need a flag that we set during auto-update
        if self._seeking:
            return
            
        value = scale.get_value()
        # Note: We need duration to know if this is valid?
        # Note: Actually scale is absolute if we set range
        if self.engine.current_track:
    ...
    def _marquee_tick(self):
        ...
        if upper <= page_size:
            # Note: No scrolling needed
            return True # Note: Keep running to check if resized
        ...
        # Note: Scroll details
        self._marquee_pos += 2 # Note: Px per tick
        
        if self._marquee_pos > (upper - page_size):
            # Note: Reached end
            self._marquee_pause = 20 # Note: Pause at end
            self._marquee_pos = 0 # Note: Reset to start (or could implement bounce/loop)
            adj.set_value(0) # Note: Snap back for now (looping is cleaner but harder with adj)
        else:
    ...
    def _update_scale_safe(self, time):
        ...
        self._seeking = False
        return False  # Note: Remove callback from idle queue to prevent memory leak
    ...
    def _on_queue_next_item(self, item: QueueItem):
        ...
        if item.source == "preview":
            # Note: Desktop does not play preview from queue yet; skip to next or leave idle
            return
    ...
    def play_track(self, track):
        ...
        if url:
             ...
             # Note: Show cover art and update labels
             ...
             # Note: Start/reset marquee
             if self._marquee_source:
                 GLib.source_remove(self._marquee_source)
             
             self._marquee_pos = 0
             self._marquee_pause = 20 # Note: Initial pause (2s at 100ms)
             self._marquee_source = GLib.timeout_add(100, self._marquee_tick)
             # Note: Wait for MPV to load and get actual duration
             GLib.timeout_add(100, self._update_duration_range, track)
             # Note: Start background download to cache for cover art
             self._cache_track_async(track)
             # Note: Try to update cover art (will retry if not cached yet)
            ...
            try:
                ...
                if self.lib_manager.cache:
                    ...
                    if self.lib_manager.provider.download_file(remote_key, temp_path):
                        ...
                        # Note: Trigger cover update now that file is cached
    ...
    def _update_duration_range(self, track):
        ...
        if duration and duration > 0:
            ...
            rng = (0, 300)  # Note: Fallback
            
        self.progress_scale.set_range(*rng)
        self.mini_scale.set_range(*rng)

        # Note: Update time label with new duration
        total = self.progress_scale.get_adjustment().get_upper()
        ...
        return False  # Note: Remove from timeout queue
    ...
    def update_cover_art(self, track, retry_count=0):
        ...
        try:
            ...
            # Note: Get local file path (from cache or download)
            local_path = None
            ...
            if not local_path:
                # Note: File not cached yet, retry after a delay (max 3 attempts)
                if retry_count < 3:
                    GLib.timeout_add(2000, self._retry_cover_update, track, retry_count + 1)
                else:
                    # Note: Fallback trigger online fetch
                    self._fetch_online_cover(track)
                    
                self.cover_art.set_from_icon_name("emblem-music-symbolic")
                return
            
            # Note: Check local cover cache FIRST (smart fetch result)
            ...
            # Note: Extract cover art from file
            cover_data = AudioProcessor.extract_cover_art(local_path)
            
            if cover_data:
                # Note: Save to temp file and load into gdkpixbuf
                with tempfile.NamedTemporaryFile(delete=False, suffix='.jpg') as tmp:
                    tmp.write(cover_data)
                    tmp_path = tmp.name
                
                try:
                    # Note: Load and scale image
                    pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(
                        tmp_path, 48, 48, True
                    )
                    self.cover_art.set_from_pixbuf(pixbuf)
                ...
                # Note: No cover found, use default
                self.cover_art.set_from_icon_name("emblem-music-symbolic")
                self.mini_cover.set_from_icon_name("emblem-music-symbolic")
            ...
            # Note: Fallback trigger online fetch if not found locally
            self._fetch_online_cover(track)
    ...
    def _show_cover_viewer(self, track):
        ...
        try:
            ...
            # Note: Create dialog immediately so user sees something happening
            # Note: Create dialog
            dialog = Adw.Window()
            dialog.set_transient_for(self)
            dialog.set_modal(True)
            dialog.set_title(f"{track.title}")
            dialog.set_default_size(350, 420) # Note: Increased height slightly for header
            
            # Note: Add ESC key handler
            key_controller = Gtk.EventControllerKey()
            ...
            # Note: Cleanup sync ref on close
            def on_dialog_close(d):
                pass
            dialog.connect("close-request", on_dialog_close)
            
            # Note: Wrapper for headerbar + content
            window_content = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
            dialog.set_content(window_content)
            
            # Note: Header bar (provides title + close button on top right)
            header = Adw.HeaderBar()
            header.set_show_end_title_buttons(True)
            window_content.append(header)
            
            # Note: Create content with proper layout
            main_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=12)
            main_box.set_margin_start(12)
            main_box.set_margin_end(12)
            main_box.set_margin_top(0) # Note: Header handles top spacing
            ...
            # Note: Loading/status overlay (stack)
            # Note: For simplicity using just the image widget, if it fails we show icon.
            ...
            # Note: Bottom controls bar - use centerbox to center playback controls
            controls_bar = Gtk.CenterBox()
            controls_bar.set_vexpand(False)
            
            # Note: Close button (start widget) - removed (moved to headerbar)
            # Note: Close_btn = GTK.button(label="close") ...
            
            # Note: Playback controls (center widget)
            playback_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=4)
            # Note: ... (Playback buttons added to playback_box below) ...
            
            # Note: We need to reconstruct playback_box here to append it correctly
            # Note: But wait, looking at the code flow, playback_box is filled line by line.
            # Note: I will define it here.
            ...
            # Note: Play/pause button (logic simplified)
            ...
            def toggle_with_update(btn):
                ...
                # Note: Update icon immediately based on resulting state
                if self.engine.is_playing:
                     play_pause_btn.set_icon_name("media-playback-pause-symbolic")
                else:
                     play_pause_btn.set_icon_name("media-playback-start-symbolic")
            
            # Note: Also poll/update periodically
            ...
            # Note: Now try to load the cover art asynchronously
            def load_cover_task():
                try:
                    # Note: Check smart cache FIRST
                    covers_dir = os.path.join(os.path.expanduser(DEFAULT_CACHE_DIR), "covers")
                    local_cover_path = os.path.join(covers_dir, f"{track.id}.jpg")
                    ...
                    if not local_path:
                        # Note: Schedule update to show "no file" or trigger download?
                        # Note: For now, just show default
                    ...
                    # Note: Save to temp file and load
                    with tempfile.NamedTemporaryFile(delete=False, suffix='.jpg') as tmp:
                        tmp.write(cover_data)
                        tmp_path = tmp.name
            ...
            # Note: Show dialog first
            dialog.present()
            
            # Note: Then start loading
            import threading
    ...
    def _update_viewer_image(self, image_widget, path):
        ...
        try:
             # Note: Paintable from file
             file = Gio.File.new_for_path(path)
             texture = Gdk.Texture.new_from_file(file)
             image_widget.set_paintable(texture)
             # Note: Remove temp file?
             # Note: It's tricky with async loading, might need cleanup later.
             # Note: For now we leak a small temp file in /tmp, usually cleaned by OS.
             # Note: Correct way pass the temp path to a cleanup thread or list.
        except Exception as e:
    ...
    def _set_viewer_default(self, image_widget, reason):
         ...
         # Note: GTK.picture doesn't take icon names easily, need to check docs.
         # Note: Actually it can take a paintable. or we can just replace the widget?
         # Note: Changing widget is disruptive.
         # Note: Let's try to load the icon theme into a texture? hard.
         # Note: Simpler use a place holder file or just leave it empty?
         # Note: Or maybe just use a separate image widget if picture fails?
         # Note: Let's try to find a system icon path or just use no paintable.
         
         # Note: GTK.picture is for displaying content.
         # Note: If we want an icon, we might need a GTK.image inside or instead.
         # Note: But we used GTK.picture above.
         
         # Note: Fallback just don't show anything in picture, and update label?
         pass

    def _toggle_playback(self):
        """Toggle play/pause from mini-player."""
        # Note: Pause() method toggles pause state
        self.engine.pause()
    ...
    def _update_mini_player_button(self, dialog):
        ...
        return False  # Note: Don't repeat
    ...
    def _retry_cover_update(self, track, retry_count):
        ...
        return False  # Note: Don't repeat timeout
    ...
    def _update_ui_state(self, state):
        ...
        if state == 'playing':
            ...
            self.play_btn.set_icon_name("media-playback-start-symbolic")
        return False  # Note: Remove from idle queue if called via glib.idle_add
    ...
    def _poll_engine_state(self):
        ...
        return True  # Note: Keep timer running
    ...
    def play_previous(self, *args):
        ...
        # Note: Standard behavior if > 3s played, restart song. else go to prev.
    ...
    def _init_player_ui(self, container, config):
        ...
        # Note: Initialize library manager
        self.lib_manager = LibraryManager()
        
        # Note: Create main content box (horizontal split)
        content_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL)
        content_box.add_css_class("content-split")
        content_box.set_vexpand(True)
        container.append(content_box)

        # Note: Create tab view (left side, expansive via hexpand)
        self.tab_view = TabView()
        self.tab_view.set_hexpand(True)
        content_box.append(self.tab_view)
        
        # Note: Create all songs tab (main library view)
        ...
        # Note: Click on library to unfocus search
        ...
        # Note: Create favourites tab (filtered library view)
        ...
        # Note: Create queue sidebar (right side)
        # Note: We wrap it in a box to include a separator
        self.sidebar_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL)
        self.sidebar_box.add_css_class("sidebar-panel")
        self.sidebar_box.set_vexpand(True) # Note: Ensure it fills height
        self.sidebar_box.set_visible(False) # Note: Hidden by default
        ...
        # Note: Register queue change callback for sidebar visibility
        self.queue_manager.add_change_callback(self._update_queue_sidebar_visibility)
        
        # Note: Search bar (at bottom)
        ...
        # Note: ESC key to unfocus and clear
        ...
        # Note: Connect search entry to active library view filter
    ...
    def _on_search_changed(self, entry):
        ...
        # Note: Apply search to appropriate library view
    ...
    def _on_album_activated(self, album_obj, tracks):
        ...
        # Note: Add as A closable page
        # Note: Position it next to favorites
    ...
    def _unfocus_search(self):
        ...
        # Note: Set focus to main window (removes focus from search)
        self.set_focus(None)
    ...
    def _on_wizard_closed(self, wizard):
        ...
        # Note: Check if config now exists
        config = self._load_config()
        if config:
            # Note: Config was created - reload the window
            print("✓ Setup complete! Reloading player...")
            # Note: TODO reload the main UI without restarting
            # Note: For now, just show a message
            self.set_title("Soundsible - Please restart the app")
        else:
            # Note: Wizard was cancelled - close app
    ...
    def show_sync_qr(self):
        ...
        local_ip = "localhost"
        try:
            # Note: 1. Try to find tailscale IP
            import subprocess
        ...
        try:
            ...
            # Note: Smart resolver pack all available network endpoints
            ...
            if ts_url:
                ...
                # Note: Add a separator or extra spacing
    ...
    def refresh_library(self, new_metadata=None):
        ...
        if new_metadata:
            ...
            def _do_instant_refresh():
                ...
                # Note: Update album grid
                if hasattr(self, 'album_grid'):
                    albums = self.lib_manager.db.get_albums()
                    self.album_grid.set_albums(albums)
                
                # Note: Update any open album tabs
                if hasattr(self, 'tab_view'):
                    # Note: Tabview might have dynamic album pages
                    # Note: We iterate through all pages and refresh if they are libraryviews
                    # Note: This is cleaner than hardcoding tab names
                    for page_name in self.tab_view.get_page_names():
                        page = self.tab_view.get_page(page_name)
                        if isinstance(page, LibraryView):
                            # Note: Filter tracks if the view has an album filter
                            if page.album_filter:
                                ...
                                # Note: Already handled above, but for consistency details
            ...
            # Note: Fallback to standard background sync for non-immediate updates
