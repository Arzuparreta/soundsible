...
export class UI {
    ...
    static init() {
        ...
        this.dom = {
            ...
            npDownloadWrap: d('np-download-wrap'),
            npDownloadBtn: d('np-download-btn'),
        ...
        /** Flushes pending slide cleanup when a new navigation starts (rapid root switches). */
        this._slideCleanupPending = null;
    ...
    static updateNowPlaying(track, isPlaying) {
        ...
        if (npDl) npDl.classList.toggle('hidden', !isYtdlpPreviewStreamTrack(track));
    ...
    static isDiscoverSearchActive() {
        ...
        return this.currentView === 'discover' && !!(input && (input.value || '').trim());
    }
    ...
    static showView(viewId, saveToHistory = true) {
        ...
        if (typeof this._slideCleanupPending === 'function') {
            this._slideCleanupPending();
            this._slideCleanupPending = null;
        }
        ...
        const forceEndSlide = () => {
            if (slideDone) return;
            slideDone = true;
            targetView.removeEventListener('transitionend', onTransitionEnd);
            if (UI._showViewCleanupTimer) {
                clearTimeout(UI._showViewCleanupTimer);
                UI._showViewCleanupTimer = null;
            }
            if (UI._slideCleanupPending === forceEndSlide) {
                UI._slideCleanupPending = null;
            }
            applySlideEnd();
        };
        ...
        const finishNormal = () => {
            if (slideDone) return;
            if (UI._viewTransitionToken !== transitionToken) return;
            slideDone = true;
            targetView.removeEventListener('transitionend', onTransitionEnd);
            if (UI._showViewCleanupTimer) {
                clearTimeout(UI._showViewCleanupTimer);
        ...
        this._slideCleanupPending = forceEndSlide;
        ...
        requestAnimationFrame(() => {
            ...
            requestAnimationFrame(() => {
                ...
                targetView.addEventListener('transitionend', onTransitionEnd);
                this._showViewCleanupTimer = setTimeout(() => finishNormal(), 450);
            });
        });
    ...
    static initGlobalListeners() {
        ...
        if (npDlBtn) {
            npDlBtn.addEventListener('click', (e) => {
                e.stopPropagation();
        ...
        if (omniTrack && omniProgressBar) {
            attachSeekBar(omniTrack, omniProgressBar, {
                dragRef: omniDragRef,
                durationState,
                onScrub: (pct, cur, dur) => updateTimeLabels(pct, cur, dur)
            });
        }
    ...
    static initGestures() {
        ...
        // Note: No-action zones do not trigger row default action (e.g. play) on tap
    ...
    static initOmniGestures() {
        ...
        const endHold = (e) => {
            ...
            // Note: Require release within island bounds horizontally so extended hitbox / fat-finger margin does not map to prev/next
            const isWithinIslandX = touch.clientX >= rect.left && touch.clientX <= rect.right;
            if (this._isHolding && !this.isBlooming && isInside && this.isIslandActive && isWithinIslandX) {
    ...
    static showActionMenu(trackId) {
        ...
        if (actionShare) actionShare.classList.toggle('hidden', !getYouTubeWatchUrlForTrack(track));
    ...
    static async showMetadataEditor(id) {
        await this._metadataEditorInit;
        this.metadataEditor?.show(id);
    }

    static async hideMetadataEditor() {
        await this._metadataEditorInit;
