...
function renderPlaylistDetail() {
    ...
    if (coverEl) {
        const coverTrack = tracks.length
            ? resolvePlaylistCoverTrack(name, trackIds, library, state.librarySettings)
            : null;
        if (coverTrack) {
        ...
        if (canPickCover) {
            ...
            const openPicker = () => {
                window.openPlaylistCoverPicker?.(name, tracks);
            };
...
function init() {
    try {
        ...
        DesktopUI.showView = (viewId) => {
            ...
            if (viewId === 'discover') {
                ...
                setTimeout(() => {
                    if (DesktopUI.currentView !== targetView) return;
        ...
        (function initDesktopClickDelegation() {
            ...
            app.addEventListener('click', (e) => {
                ...
                // Surfaces without a row-level "selected" workflow should play on single click,
                // otherwise the row advertises itself as clickable but does nothing.
                if (id && (DesktopUI.currentView === 'podcast' || row.dataset.deezerSurface === '1')) playTrack(id);
            });
        })();
        ...
        DesktopUI.showView(store.hasLocalLibrarySignal() ? 'home' : 'discover');
    ...
    void connectionManager.findActiveHost([...new Set(endpoints)].filter(Boolean)).then(() => {
