...
export const Discover = {
    ...
    _renderShelfCard(r) {
        const thumb = (r.thumbnail || '').replace(/"/g, '%22');
        const coverStyle = r.thumbnail ? `background-image: url(${escapeCssUrl(r.thumbnail)})` : '';
        return `
            <div class="discover-card discover-result-row relative" data-video-id="${esc(r.id)}" data-title="${esc(r.title)}" data-artist="${esc(r.artist || r.channel || '')}" data-duration="${r.duration || 0}" data-webpage-url="${esc(r.webpage_url || '')}" data-thumbnail="${esc(thumb)}">
            ...
            if (this._sectionsEl) {
                ...
                this._sectionsEl.innerHTML = `
                    <div class="text-center py-6 text-sm text-[var(--text-dim)]">
                        <p class="mb-3">Connect Spotify (in Settings) or add a Last.fm API key below.</p>
