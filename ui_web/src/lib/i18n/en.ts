/** Canonical English dictionary — also the source of the `Dict` type. */
export const en = {
  searchHome: {
    "artists": "Artists to discover",
    "albums": "Albums for you",
    "songs": "Songs you might like",
    "popularArtists": "Popular artists",
    "popularAlbums": "Popular albums",
    "related": "Related to {artist}",
    "seeAll": "See all",
    "back": "Back to Search",
    "previous": "Previous",
    "next": "Next",
    "preparing": "Finding artists and albums for you…",
    "empty": "Search for music to start exploring.",
    "error": "Some recommendations could not be loaded."
},
  musicExplorer: {
    change: 'Change session',
    mixWith: 'Mix with…',
    changeShort: 'Change…',
    changing: 'Preparing session from {title}',
    changeFailed: 'Could not change session',
    retryChange: 'Retry',

    title: 'Music',
    explore: 'Explore',
    request: 'Add to session',
    requestAll: 'Add to session',
    requestShort: 'Add',
    reference: 'Mix into session',
    references: 'Session',
    referenceHint: 'Music that guides the DJ’s next selections.',
    referenceEmpty: 'Choose music',
    addReference: 'Mix into session',
    viewReferences: 'View all session music',
    allMusic: 'All music',
    libraryOnly: 'In your library',
    types: 'Result type',
    all: 'All',
    youtube: 'YouTube',
    cancelPlacement: 'Cancel selection',
    placing: 'Choose music for this position in Route',
    edit: 'Edit',
    done: 'Done',
    up: 'Move up',
    down: 'Move down',
    newPlaylist: 'New playlist',
    collectionDone: '{count} songs requested',
    collectionFailed: 'Could not resolve: {titles}',
    requesting: 'Preparing your requests…',
    partial: 'Some sources could not answer. Available results are shown.',
    openAlbum: 'Go to album',
    playNow: 'Play now',
    referencePicking: 'Mix into session',
    filter: 'Filter',
    downloaded: 'Downloaded',
    catalog: 'Explore artist',
    library: 'In your library',
  },
  musicList: {
    paused: 'Paused',
    move: 'Move',
    moveUp: 'Move up',
    moveDown: 'Move down',
    done: 'Done',
  },

  brand: {
    soundsible: 'Soundsible',
    soundsibleWeb: 'Soundsible Web',
  },
  nav: {
    menu: "Menu",
    yourMusic: "Your music",
    explore: "Explore",
    application: "Application",
    bottomBar: "Bottom bar",
    bottomBarHint: "Choose 3–5 destinations. Changes apply to this browser or installation.",
    position: "Position {position}",
    addDestination: "Add destination",
    removeDestination: "Remove {name}",
    restoreDefaults: "Restore defaults",

    more: 'More',
    mobile: 'Main navigation',
    search: 'Search',
    favourites: 'Favourites',
    playlists: 'Playlists',
    podcasts: 'Podcasts',
    live: 'Live',
    downloads: 'Downloads',
    settings: 'Settings',
    library: 'Library',
    shortcuts: 'Quick access',
  },
  live: {
    title: 'Live',
    meta: 'Sessions from the Soundsible community',
    goLive: 'Go live',
    end: 'End session',
    onAir: 'On air',
    chat: 'Chat',
    chatEmpty: 'Messages appear here from the moment you join.',
    chatPlaceholder: 'Write a message',
    send: 'Send',
    listeners: '{count} listeners',
    waiting: 'About to start',
    broadcasting: 'Your Soundsible master is live.',
    startPlaying: 'Start playback when you are ready.',
    directory: 'Live now',
    refresh: 'Refresh',
    empty: 'There are no live sessions right now.',
    unavailable: 'Soundsible Community is unavailable or not configured.',
    listen: 'Listen live',
    listening: 'Listening live',
    listenFailed: 'The live audio could not be connected.',
    connectingAudio: 'Connecting audio',
    reconnectingAudio: 'Recovering audio',
    publishFailed: 'Audio publishing failed',
    openSecure: 'Go live on the secure station',
    share: 'Share room',
    linkCopied: 'Link copied',
    ownRoom: 'Your room — open the shared link on another device to check it',
    service: {
      loading: 'Connecting to Soundsible Community…',
      disabled: 'Community is disabled on this station.',
      invalid: 'The custom Community relay address is invalid.',
      unavailable: 'The Community relay cannot be reached.',
      capacity: 'The live directory is currently at capacity.',
      secure_context: 'Live broadcasting needs HTTPS. Open this station through its secure Tailscale hostname or HTTPS domain.',
      reconnecting: 'The live connection is recovering…',
      publish_failed: 'Soundsible could not publish the live audio.',
      listen_failed: 'Soundsible could not restore the live audio.',
      graph_lost: 'The mixer stopped and took the broadcast with it. Reload Soundsible to go back on air.',
    },
    onBreak: 'Back in a moment · {time}',
    breakHint: 'The DJ paused the music.',
    hostBreak: 'You have been silent on air for {time}.',
    status: {
      waiting: 'Waiting',
      live: 'Live',
      paused: 'On a break',
      reconnecting: 'Reconnecting',
    },
  },
  common: {
    loadFailed: 'Could not load. Please try again.',
    play: 'Play',
    pause: 'Pause',
    next: 'Next',
    prev: 'Previous',
    cancel: 'Cancel',
    save: 'Save',
    close: 'Close',
    back: 'Back',
    more: 'More options',
    retry: 'Retry',
    delete: 'Delete',
    rename: 'Rename',
    loading: 'Loading…',
    preparingProgress: 'Preparing… {progress}%',
    preparingProgressEta: 'Preparing… {progress}% · {eta}s',
    online: 'Connected',
    offline: 'Offline',
    continue: 'Continue',
    undo: 'Undo',
  },
  omnibar: {
    nothingPlaying: 'Nothing playing',
    engineConnected: 'Engine connected',
    mute: 'Mute',
    unmute: 'Unmute',
    volume: 'Volume',
    loading: 'Loading…',
    buffering: 'Buffering…',
    bufferingSlow: 'Buffering — {link} Mbps, this track needs {needed}',
    reconnecting: 'Reconnecting…',
    findingMore: 'Finding more music…',
    needsGesture: 'Tap play to resume',
    unavailable: 'Unavailable — tap to retry',
  },
  nowPlaying: {
    playing: 'Playing',
    modeLabel: 'NORMAL',
    modeSelector: 'Player mode',
    mobilePanels: 'NORMAL panels',
    panel: { browser: 'Music', stage: 'Player', queue: 'Queue' },
    openSearch: 'Open search',
    nothingPlaying: 'Nothing playing',
    seekLabel: 'Seek',
    shuffle: 'Shuffle',
    repeat: 'Repeat',
    removeFav: 'Remove from favourites',
    addFav: 'Add to favourites',
    saveToLibrary: 'Save to library',
    showLyrics: 'Show lyrics',
    showCover: 'Show cover art',
    movePanel: 'Move {panel} panel',
    layoutPanel: 'Panel layout',
    movePanelLeft: 'Move left',
    movePanelRight: 'Move right',
    resizePanels: 'Resize adjacent panels',
    changeLayout: 'Change layout',
    layoutWorkspace: 'NORMAL layout',
    layoutBalanced: 'Balanced',
    layoutPlayer: 'Player focus',
    layoutExplore: 'Explore',
    layoutQueue: 'Queue focus',
    resetLayout: 'Reset layout',
    queue: 'Up next',
    clearQueue: 'Clear',
    clearManualQueue: 'Clear requests',
    removeFromQueue: 'Remove from queue',
    nowPlayingSection: 'Now playing',
    manualQueue: 'Your queue',
    laneHintCurrent: 'The song playing right now.',
    laneHintManual: 'What you asked for by hand. It plays before anything else.',
    laneHintRadio: 'Picked by the radio as it goes.',
    autoplayQueue: 'Autoplay',
    radioQueue: 'Radio',
    autoQueue: 'DJ',
    queueEmpty: 'Nothing else is lined up.',
    continuationSection: 'Then',
    laneHintContinuation: 'What plays once your requests run out.',
    contextUntitled: 'This selection',
    contextKind: {
      album: 'Album',
      playlist: 'Playlist',
      artist: 'Artist',
      library: 'Library',
      favourites: 'Favourites',
      search: 'Selection',
      single: 'Song',
      podcast: 'Podcast',
    },
    contextOpen: 'Open {name}',
    contextRemove: 'Remove {name} from the queue',
    contextRepeats: 'repeats',
    contextShuffled: 'shuffled',
    autoplayOn: 'On · similar music once the rest runs out',
    autoplayPreparing: 'On · finding similar music',
    autoplayOff: 'Off',
    radioActiveAria: 'Radio is active. Click to stop the radio.',
    stopRadioTitle: 'Stop radio?',
    stopRadioMsg: 'The current track will keep playing, but the rest of the radio mix will be cleared.',
    stopRadioConfirm: 'Stop radio',
  },
  autoMode: {
    aria: 'DJ autonomous player',
    label: 'DJ',
    enter: 'Enter DJ',
    startDj: 'Start a DJ session',
    startDjHint: 'Auto will choose the first track',
    noSeed: 'DJ could not find a track to start with.',
    exit: 'Exit DJ',
    changeProfile: 'Change profile. Current: {profile}',
    profile: {
      familiar: 'Familiar',
      balanced: 'Balanced',
      explore: 'Explore',
    },
    reason: {
      favorite: 'From your favourites',
      library: 'From your library',
      related: 'Following the thread of {title}',
      nodeArtist: 'Exploring from {artist}',
      node: 'From your music map',
      chart: 'Trending right now',
      artist: 'More from {artist}',
    },
    agent: {
      heard: 'Heard you: {note}',
      looking: 'Looking for “{name}”…',
      noMatch: 'Nothing here answers to “{name}”',
      searching: 'Reading “{title}” for somewhere to go…',
      redrawing: 'Redrawing the route · {note}',
      steered: 'Route redrawn · {note}',
      queued: 'Cued {tracks} · {related} related, {node} from your map, {local} from your library',
      retrying: 'Nothing new landed across {related} related, {node} map and {local} library tracks · trying again',
      placing: 'Fitting “{title}” into the route…', placed: '“{title}” is in the route',
      placedFallback: '“{title}” is in the route; the mix will adjust when it reaches it',
      repairing: 'Rebuilding the mix around your songs…',
      repaired: 'Route re-seamed · your songs kept their places',
      repairFailed: 'Could not rebuild the mix · the route is untouched',
      repairSkipped: 'The route changed while it was being rebuilt · nothing was moved',
      immediateQueued: '“{title}” will enter as soon as the current mix settles',
      mixingImmediate: 'Preparing the mix into “{title}”…',
      openingSource: 'Choosing how to open the session…',
      openedSource: 'Opening the session with “{title}”',
      openingFailed: 'No playable opening was found in this source',
    },
    upNext: 'Up next',
    panel: {
      browser: 'Music',
      booth: 'Booth',
      stage: 'Now playing',
      route: 'Route',
    },
    source: {
      title: 'Sources', add: 'Mix into session', selection: 'Selection', empty: 'Drag music here to steer the session.',
      libraryActions: 'Use the library as a source', remove: 'Remove the {title} source',
      routeEmpty: 'Add a track to the route or resume playback',
      added: 'Steering the session from “{title}”',
      already: '“{title}” is already in the session',
    },
    route: {
      pinned: 'Pinned', actions: 'Actions for {title}', useAsSource: 'Mix into session',
      add: 'Add', insertBefore: 'Insert a track before {title}', fixed: 'Fixed', placed: 'Placed', bridge: 'Bridge',
      remove: 'Remove from route', avoidSession: 'Avoid during this session',
      avoided: '“{title}” will not return during this session',
      addToRoute: 'Add {title} to the route', addHere: 'Add {title} here',
      fix: 'Fix mix', fixing: 'Fixing…',
      fixHint: 'Rebuild the mix around the songs you placed',
      mixPending: 'Some joins have no transition planned yet',
      moved: '“{title}” moved · the mix still needs rebuilding',
      dropHere: 'Drop to place here',
      preparingFrom: 'Building the route from “{title}”',
      preparing: 'Building the DJ route',
      preparingWhilePlaying: 'The track keeps playing while the DJ fits what comes next.',
      preparingHint: 'The route will appear here as soon as the mix is ready.',
      retrying: 'Looking for another route',
      warming: 'Finding what fits with “{title}”',
      warmingHint: 'The first route from a new source can take a little longer. Playback will continue.',
      exhausted: 'No new tracks are available to continue this session.',
      retryingHint: 'The search could not complete. The DJ will try again.',
    },
    workspace: {
      aria: 'DJ panels',
      changeLayout: 'Change DJ layout',
      layoutTitle: 'DJ layout',
      layoutBalanced: 'Balanced',
      layoutStage: 'Player focus',
      layoutBooth: 'Booth focus',
      layoutRoute: 'Route focus',
      resetLayout: 'Reset layout',
    },
    booth: {
      aria: 'DJ booth controls',
      now: 'On air', next: 'Next', bpm: 'BPM', in: 'in {time}',
      energy: 'Energy', hold: 'Hold',
      energyDown: 'Ease off', energyUp: 'Push',
      crate: 'Crate', crateDeep: 'Dig deep', crateKnown: 'Sure things',
      opening: 'Picking somewhere to start',
      openingHint: 'Reading your library and what you have been playing',
    },
    mobile: {
      controls: 'DJ panels',
      booth: 'Booth',
      route: 'Route',
      routeEmpty: 'The DJ is still preparing what comes next',
      closePanel: 'Close DJ panel',
    },
    note: {
      quoted: '“{text}”',
      direction: 'a new heading',
      added: '“{title}” into the set',
      dropped: '“{title}” dropped',
      repaired: 'The mix was rebuilt around your songs',
      energy: { down: 'easing off', hold: 'holding the energy', up: 'more push' },
      crate: { deep: 'deeper cuts', hold: 'an even spread', known: 'safer ground' },
      profile: { familiar: 'safer ground', balanced: 'an even spread', explore: 'deeper cuts' },
      dj: {
        adaptive: 'mix it however each pair wants',
        long_blend: 'long blends',
        cuts_drops: 'cuts and drops',
        open_format: 'open format',
      },
    },
    dj: {
      label: 'DJ', change: 'Change', changeCurrent: 'Change DJ. Current: {dj}',
      choose: 'How the booth mixes', chooseHint: 'This is the hands on the decks, not the music.',
      adaptive: 'Adaptive', adaptiveTrait: 'Reads each pair and picks the mix',
      longBlend: 'Long blend', longBlendTrait: 'Harmonic · 16–32 bars · subtle effects',
      cutsDrops: 'Cuts & drops', cutsDropsTrait: 'Short runs · marked changes · echo cuts',
      openFormat: 'Open format', openFormatTrait: 'Wide jumps · bridge tracks · structural changes',
      direction: 'Music direction',
      tellDj: 'Tell the DJ…',
      directionHint: 'Tell the booth where to take this. It handles the mixing.',
      commandPlaceholder: 'play oliver heldens · darker · nothing from the eighties',
      commandAria: 'Tell the booth where to take the session', send: 'Send',
      route: 'Prepared route', preparing: 'Preparing', mixing: 'Mixing', analysing: 'reading',
      cued: 'Cued', appliesNext: 'Lands on the next track. What is cued stays cued.',
      promote: 'Play {title} sooner',
      routePanelAria: 'Add a track to the route',
      searchPlaceholder: 'Track or artist', searching: 'Searching…',
      searchEmpty: 'Find a track for the route', routeAction: 'Add to route', mixNow: 'Mix now',
      technique: {
        long_blend: 'Long blend', bass_swap: 'Bass swap', filter_blend: 'Filter blend',
        echo_cut: 'Echo cut', structural_fade: 'Structural change', safe_fade: 'Smooth fade',
      },
    },
  },
  modeChange: {
    toNormalTitle: 'Switch to NORMAL',
    podcastMessage: 'DJ only mixes music. Playing this podcast will end the DJ session.',
    radioMessage: 'Radio and DJ are different sessions. Starting Radio will end the DJ session.',
    playPodcast: 'Switch and play',
    startRadio: 'Switch and start Radio',
  },
  autoplay: {
    reason: 'Similar to what you were listening to',
  },
  library: {
    searchAction: 'Search library',
    title: 'Your library',
    favourites: 'Favourites',
    playlists: 'Playlists',
    podcasts: 'Podcasts',
    downloads: 'Downloads',
    songs: 'Songs',
    albums: 'Albums',
    artists: 'Artists',
    albumSortTitle: 'Sort albums',
    albumSort: {
      newest: 'Recently added',
      alphabeticalByName: 'A–Z',
      alphabeticalByArtist: 'By artist',
      byYear: 'By year',
      frequent: 'Most played',
    },
    albumFilterTitle: 'Show only',
    albumFilterAll: 'All albums',
    albumFilterByGenre: 'Genre',
    albumFilterByYear: 'Year',
    sortRecent: 'Recent',
    sortAZ: 'A–Z',
    sortFavFirst: 'Favourites first',
    sortTitle: 'Sort library',
    filterTitle: 'Show only',
    filterDownloaded: 'Downloaded',
    searchLibrary: 'Search songs and artists',
    clearSearch: 'Clear library search',
    noSearchResults: 'No songs or artists match this search.',
    resultTrack: 'Song',
    resultArtist: 'Artist',
    openArtist: 'Open {artist}',
    artistTrackCount: '{count} songs',
    emptyArtists: 'No artists yet.',
    emptyAlbums: 'No albums yet. A song lands on one as soon as it arrives tagged with a record.',
    emptyAlbumFilter: 'No albums match this filter.',
    emptyLibrary: 'Your library is empty. Save anything you find in Search — downloading it is a separate step.',
    unreachable: "Couldn't reach your station, so this may not be everything.",
    unreachableEmpty: "Couldn't reach your station. Your library is still there.",
    retry: 'Try again',
  },
  /** Having a song, and having its bytes: two steps, one control. */
  collection: {
    save: 'Save to your library',
    unsave: 'Remove from your library',
    download: 'Download',
    downloading: 'Downloading…',
    owned: 'Downloaded',
    resolving: 'Finding a source…',
  },
  favourites: {
    title: 'Favourites',
    empty: 'Nothing marked yet. The heart singles a song out from the rest of your library.',
  },
  search: {
    placeholder: 'What do you want to play?',
    tabAll: 'All',
    tabSongs: 'Songs',
    tabArtists: 'Artists',
    tabAlbums: 'Albums',
    ytVideoTitle: 'YouTube video',
    ytVideoChannel: 'YouTube',
    noPreview: 'Could not load preview',
    addedToDownloads: 'Added to downloads',
    notSaved: 'Could not save',
    alreadyInLibrary: 'Already in your library',
    alreadyInQueue: 'Already in the download queue',
    notAddedDownloads: 'Could not add to downloads',
    ytErrorHint: 'The YouTube search could not be completed.',
    ytNoResults: 'No YouTube results.',
    ytPressEnter: 'Press Enter to search on YouTube.',
    ytDirectSection: 'Detected video',
    ytResultsSection: 'YouTube results',
    ytFallbackCta: "Not what you wanted? Look on YouTube",
    catalogErrorHint: 'The search could not be completed.',
    catalogNoResults: 'No Music results.',
    searchInYt: 'Search on YouTube',
    topResultSection: 'Top result',
    seeAllSongs: 'See all {count} songs',
    typeArtist: 'Artist',
    typeAlbum: 'Album',
    typeSong: 'Song',
    resultsSection: 'Results',
    interpretedAs: 'Showing results for {name}',
    songsSection: 'Songs',
    artistsSection: 'Artists',
    albumsSection: 'Albums',
    chooseVersion: 'Choose a version',
    ytRecentsSection: 'Recent YouTube searches',
    recentsSection: 'Recent searches',
    seedHeading: 'Search for music to get started',
    seedDesc:
      'Recommendations are generated from the artists, favourites, playlists, and songs you save or play.',
    seedAction: 'Search for songs',
    labelLibraryTrack: 'In your library',
    labelTrack: 'Song',
    labelArtist: 'Artist',
    labelAlbum: 'Album',
    labelPlaylist: 'Playlist',
    sharedSection: 'Shared with you',
    sharedUnavailable: 'This exact shared version is unavailable.',
    sharedInvalid: 'This shared Soundsible link is not valid.',
    sharedLink: 'Shared song',
  },
  settings: {
    repair: 'Repair files',
    repairTitle: 'Repair library files',
    repairMsg: 'Files that carry a video stream or an oversized cover will be rewritten with the audio untouched. This changes their ids, and playlists and favourites follow automatically.',
    repairConfirm: 'Repair',
    link: {
      label: 'Delivery',
      measured: '{where} · {mbps} Mbps measured',
      notMeasured: '{where} · not measured yet',
      scopeUnknown: 'Unknown network',
      scope: {
        local: 'This machine',
        lan: 'Local network',
        tailnet: 'Tailnet',
        remote: 'Remote',
      },
    },
    title: 'Settings',
    searchPlaceholder: 'Search settings',
    searchNoResults: 'Nothing matches “{query}”',
    searchResults: 'Search results',
    searchCount: '{count} results',
    searchCountOne: '1 result',
    /** Extra words the settings search matches, comma-separated. Never shown. */
    searchAliases: {
      theme: 'dark mode, light mode, night mode, colours, colors, appearance',
      language: 'locale, translation, languages',
      interfaceSize: 'zoom, scale, font size, text size, bigger text',
      highContrast: 'legibility, readability, visibility',
      haptics: 'vibration, vibrate, taptic',
      bottomBar: 'tab bar, navigation, tabs, shortcuts',
      autoplay: 'radio, endless, infinite, keep playing, similar music',
      learnActivity: 'history, personalisation, personalization, privacy',
      quality: 'bitrate, kbps, audio quality, file size',
      password: 'credentials, security, passphrase',
      signOut: 'log out, logout, sign off, exit',
      deviceName: 'rename, device, computer name',
      pairedDevices: 'pair, qr, phone, mobile, remote control',
      rescan: 'scan, music folder, index',
      emptyLibrary: 'delete everything, wipe, erase, reset library',
      lossless: 'flac, hi-res, hifi, upgrade quality',
      version: 'version, build, release',
      delivery: 'network, connection, bandwidth, speed, mbps',
      import: 'spotify, apple music, playlists, migrate, transfer',
      users: 'accounts, members, family, admin, invite',
    },
    close: 'Close settings',
    playback: 'Playback',
    deviceName: 'Name',
    community: 'Community',
    communityService: 'Service',
    communityStatus: 'Status',
    communityRelay: 'Relay',
    communitySource: {
      official: 'Soundsible official service',
      custom: 'Custom relay',
      disabled: 'Disabled',
    },
    communityState: {
      available: 'Available',
      disabled: 'Disabled by configuration',
      invalid: 'Invalid configuration',
      unavailable: 'Unreachable',
    },
    /* One line under each category on the index: what you will find inside. */
    blurb: {
      account: 'Your name, password and who else can sign in',
      appearance: 'Theme and language',
      accessibility: 'Interface size, contrast and vibration',
      playback: 'Autoplay and how recommendations learn',
      library: 'Sync, import, clean up and remove tracks',
      users: 'Who else can sign in, and what they are allowed to do',
      downloads: 'Audio quality and the download engine',
      devices: 'This device, paired phones and remote control',
      community: 'Official Live service or your own relay',
      about: 'Engine status and version',
      subsonic: 'Play your library in Symfonium, Amperfy, Feishin and the rest',
    },
    /* Headers used by the index and inside the submenus. */
    group: {
      connection: 'Connection',
      you: 'You',
      preferences: 'Preferences',
      system: 'Library and system',
      profile: 'Profile',
      feedback: 'Feedback',
      recommendations: 'Recommendations',
      upNext: 'Up next',
      sync: 'Sync',
      maintenance: 'Maintenance',
      updates: 'Updates',
      thisDevice: 'This device',
      network: 'On this network',
    },
    /* Footers: the quiet line under a group that says what it actually does. */
    note: {
      connection: 'Measured from the music already delivered — no test traffic. A track needs about 1.5x its own bitrate to play straight through.',
      noPassword: 'This account has no password yet',
      theme: 'System follows whatever your device is set to.',
      haptics: 'Short vibrations when you use the player controls, on devices that support them.',
      autoplay: 'When the queue runs out, keep playing music that fits what you were listening to.',
      volumeLeveling: 'Plays every song at a similar loudness, so you are not reaching for the volume between tracks. Measured once per file; nothing is re-encoded and your music is never modified.',
      sync: 'Reload re-reads the database. Rescan reads the music folder again and picks up files added by hand.',
      maintenance: 'Optimise compacts the database. Purge removes tracks whose files are gone from disk.',
      quality: 'Applies to new downloads. Higher quality means larger files.',
      updates: 'Keeps downloads working when the sources change. Leave these on unless you pin versions yourself.',
      device: 'The name other devices see when they control this player.',
      network: 'Devices signed in to this server. Control what they are playing from here.',
      community: 'Live uses the official service automatically. Environment variables are only needed to disable it or select your own HTTPS relay.',
    },
    appearance: 'Appearance',
    theme: 'Theme',
    otherThemes: 'Other themes',
    selectTheme: 'Select…',
    themeSlate: 'Slate',
    themePureBlack: 'Pure black',
    themeForestGreen: 'Forest green',
    themeDark: 'Dark',
    themeLight: 'Light',
    themeSystem: 'System',
    haptics: 'Vibration (haptics)',
    discovery: 'Discovery',
    autoplay: 'Autoplay similar music',
    volumeLeveling: 'Match volume across songs',
    volumeLevelingSearch: 'Loudness normalisation, ReplayGain, sound check',
    learnActivity: 'Learn from my activity',
    learnActivityNote:
      'Improves recommendations using your local history. Nothing is sent to third parties.',
    resetLearning: 'Reset recommendation learning',
    resetLearningConfirm: 'Your local recommendation history and feedback will be cleared.',
    resetLearningDone: 'Recommendation learning reset',
    downloads: 'Downloads',
    quality: 'Quality',
    qualityLow: 'Low',
    qualityNormal: 'Normal',
    qualityHigh: 'High',
    autoUpdateYtdlp: 'Auto-update yt-dlp',
    autoUpdateCurlCffi: 'Auto-update web compatibility (curl-cffi)',
    losslessUpgrades: 'Upgrade to lossless while idle',
    losslessStatusLabel: 'Lossless upgrades',
    losslessStatus: '{upgraded} upgraded · {pending} pending',
    losslessUnavailable: 'Identity verification unavailable',
    losslessNote:
      'Soundsible looks for a freely licensed lossless copy of your tracks and only swaps one in when its audio fingerprint matches. Most tracks never have one — "no match" is a normal outcome, not a failure.',
    losslessState: 'Status',
    losslessStateStopped: 'Stopped',
    losslessStateDisabled: 'Off',
    losslessStateWaiting: 'Waiting for an idle moment',
    losslessStateIdle: 'Nothing left to check',
    losslessStateInventory: 'Scanning your library',
    losslessStateProcessing: 'Checking a track',
    losslessStatePaused: 'Paused',
    losslessStateBudget: 'Daily limit reached',
    losslessUpgraded: 'Upgraded',
    losslessPending: 'Pending',
    losslessNoMatch: 'No lossless found',
    losslessCheckedToday: 'Checked today',
    losslessVerifier: 'Fingerprinting',
    losslessRunNow: 'Upgrade now',
    losslessRunNowHint: 'Runs immediately, without waiting for an idle moment',
    losslessPause: 'Pause',
    losslessResume: 'Resume',
    losslessCancel: 'Stop run',
    losslessProcessed: '{n} checked in this run',
    losslessRecheck: 'Re-check every track',
    losslessRecheckHint: 'Ask the providers again about tracks that came back empty',
    losslessRecheckConfirm:
      'Every track that found no lossless copy goes back in the queue and the providers are asked again. This can take a while.',
    losslessRunStarted: 'Upgrade run started',
    losslessRunCancelled: 'Upgrade run stopped',
    losslessFailed: 'Could not reach the upgrade service',
    losslessJamendo: 'Jamendo',
    losslessJamendoReady: 'Configured',
    losslessJamendoMissing: 'Client ID missing',
    losslessJamendoClientId: 'Jamendo Client ID',
    losslessJamendoHint: 'Find it inside your application in the Jamendo developer portal',
    losslessJamendoSave: 'Save Client ID',
    losslessJamendoSaved: 'Jamendo configured',
    losslessJamendoInvalid: 'Could not configure Jamendo',
    optimize: 'Optimise library',
    sync: 'Cloud sync',
    libraryCard: 'Library',
    tracks: 'Tracks',
    reload: 'Reload',
    rescan: 'Rescan files',
    purgeFiles: 'Purge missing files',
    emptyLibrary: 'Empty library',
    importCard: 'Import',
    importNote:
      'Bring your playlists from Spotify or Apple Music and match them with your library.',
    importFrom: 'Import from Spotify / Apple Music',
    device: 'Device',
    devices: 'Devices',
    pairedDevices: 'Paired devices',
    pairNote: 'Pair your phone by scanning a QR to control it and play remotely.',
    connection: 'Connection',
    engineLabel: 'Engine',
    about: 'About',
    version: 'Beta · Solid UI',
    viewDesign: 'View design system',
    language: 'Language',
    sharedLinks: 'Shared links',
    openSharedLinks: 'Open Soundsible links in this app',
    purgeTitle: 'Purge missing files',
    purgeMsg: 'Removes library tracks whose files no longer exist on disk.',
    purgeConfirm: 'Purge',
    emptyLibraryTitle: 'Empty library',
    emptyLibraryMsg: 'The ENTIRE library will be deleted. This cannot be undone.',
    emptyLibraryConfirm: 'Empty everything',
    emptyLibraryCountMsg: 'You have {count} in your library.',
    emptyLibraryCountLabel: 'Enter the number of tracks in your library to confirm',
    toast: {
      repairScanning: 'Checking what the files carry…',
      repairScanned: 'Report written to the downloads log',
      repairing: 'Repairing files…',
      repaired: 'Files repaired',
      repairFailed: 'Repair failed',
      notSaved: 'Could not save',
      qualityUpdated: 'Quality updated',
      optimizing: 'Optimising…',
      optimized: 'Library optimised',
      optimizeFailed: 'Could not optimise',
      syncing: 'Syncing…',
      synced: 'Synced',
      syncFailed: 'Could not sync',
      scanning: 'Scanning music folder…',
      scanned: 'Scan complete · {added} added · {updated} updated · {failed} failed',
      scanFailed: 'Could not scan the music folder',
      purging: 'Purging…',
      purged: 'Purged ({count})',
      purgeFailed: 'Could not purge',
      emptying: 'Emptying…',
      emptied: 'Library emptied',
      emptyFailed: 'Could not empty',
    },
  },
  accessibility: {
    title: 'Accessibility',
    open: 'Open display accessibility settings',
    intro: 'Make text, icons and controls easier to see on this device.',
    interfaceSize: 'Interface size',
    size: {
      compact: 'Compact',
      normal: 'Normal',
      large: 'Large',
    },
    highContrast: 'Enhanced contrast',
    highContrastNote: 'Makes secondary text, icons and boundaries more distinct.',
  },
  downloads: {
    title: 'Downloads',
    fallbackTitle: 'Download',
    phaseInterrupted: 'Interrupted',
    phaseFailed: 'Error',
    phaseFailedPercent: 'Error at {percent}%',
    phasePreparing: 'Preparing',
    phaseProcessing: 'Processing',
    phaseDownloading: 'Downloading',
    phasePending: 'Queued',
    failedDetail: 'The download failed.',
    ariaBack: 'Back',
    noActive: 'No active downloads',
    countActive: '{active} in progress',
    countFailed: ' · {failed} failed',
    clearErrors: 'Clear errors',
    recentSubtitle: 'Added to library',
    emptyTitle: 'Nothing in the queue',
    emptyBody: 'Add songs from Search and you will see their progress here in real time.',
    clearQueue: 'Clear queue',
    ariaRetry: 'Retry',
    ariaRemove: 'Remove',
    ariaCancel: 'Cancel',
  },
  playlists: {
    title: 'Playlists',
    new: 'New playlist',
    newPlaceholder: 'Playlist name',
    newConfirm: 'Create',
    empty: "You don't have any playlists yet.",
    ariaOptions: 'Playlist options',
  },
  playlistDetail: {
    play: 'Play',
    ariaOptions: 'Playlist options',
    ariaBack: 'Back',
    empty: 'Empty playlist.',
  },
  podcasts: {
    searchPlaceholder: 'Search podcasts…',
    yourShows: 'Your shows',
    top: 'Popular podcasts',
    subscribed: 'Subscribed',
    hint: 'Search for podcasts above and subscribe. Your shows will appear here.',
    noResults: 'No results.',
    subscribing: '…',
    subscribe: 'Subscribe',
  },
  podcastShow: {
    fallbackTitle: 'Podcast',
    ariaBack: 'Back',
    unsubscribe: 'Unfollow',
    downloadedOnly: 'Downloaded only',
    empty: 'No episodes.',
    ariaPlay: 'Play episode',
    ariaDownload: 'Download episode',
    ariaDownloaded: 'Downloaded',
    durationMin: '{m} min',
  },
  migrate: {
    spotify: 'Spotify',
    spotifyHint:
      'Export your data from Spotify (or use an export tool) and upload the JSON playlist file.',
    apple: 'Apple Music',
    appleHint: 'In Apple Music, export the playlist as a text/CSV file and upload it here.',
    defaultName: 'Imported',
    title: 'Import library',
    chooseFile: 'Choose a file…',
    orPaste: 'or paste the contents',
    placeholderSpotify: '{ "tracks": [ … ] }',
    placeholderApple: 'Title,Artist,Album…',
    analyzing: 'Analysing…',
    preview: 'Preview matches',
    statLine: '{matched} of {total} tracks found in your library',
    chipAuto: 'confident',
    chipReview: 'to review',
    chipUnmatched: 'not found',
    playlistName: 'Playlist name',
    matches: 'Matches ({count})',
    deselectAll: 'Deselect all',
    selectAll: 'Select all',
    unmatched: 'Not found ({count})',
    unmatchedHint:
      "These aren't in your library, so they won't be imported. Search for them in Search to download them.",
    dash: '—',
    back: 'Back',
    create: 'Create playlist ({count})',
    confidenceAuto: 'Confident match',
    confidenceReview: 'Worth reviewing',
    toast: {
      readFailed: 'Could not read the file',
      empty: 'Add a file or paste the export contents',
      badFormat: 'Could not read the export. Is it the right format?',
      libraryEmpty: 'Your library is empty: download music before importing.',
      previewFailed: 'Could not preview the import',
      nameRequired: 'Name the playlist',
      selectOne: 'Select at least one track',
      importing: 'Importing…',
      created: 'Playlist "{name}" created ({count})',
      dupe: 'A playlist with that name already exists',
      createFailed: 'Could not create the playlist',
    },
  },
  artist: {
    ariaBack: 'Back',
    play: 'Play',
    shuffle: 'Shuffle',
    empty: 'You have no tracks by this artist in your library.',
    discover: 'Discover',
    library: 'My library',
    topTracks: 'Top tracks',
    albums: 'Albums',
    singlesEps: 'Singles & EPs',
    related: 'Fans also like',
    fans: '{n} fans',
    inLibraryCount: '{n} in library',
    notThisArtist: 'Not this artist?',
    noCatalogData: 'No catalog data available for this artist.',
    noTopTracks: 'No top tracks found.',
    noAlbums: 'No albums found.',
    noRelated: 'No related artists found.',
  },
  album: {
    ariaBack: 'Back',
    play: 'Play',
    shuffle: 'Shuffle',
    discover: 'Discover',
    library: 'My library',
    empty: 'No tracks from this album in your library.',
    year: '{year}',
    tracks: '{n} tracks',
    noTracklist: 'No tracklist available for this album.',
  },
  placeholder: {
    overlayHeading: 'Overlay manager',
    overlayText:
      'This sheet lives in the overlay registry. Closing it — or navigating to another view — cleans up its DOM, listeners, and reactive scope by itself. The historic Discover leak is impossible by design.',
    overlayClose: 'Close',
    statusOnline: 'engine ●',
    statusOffline: 'offline ○',
    tryOverlay: 'Try overlay',
    notFoundTitle: 'Not found',
    notFoundBlurb: "This route doesn't exist.",
  },
  trackActions: {
    playNext: 'Play next',
    addToQueue: 'Add to queue',
    addToPlaylist: 'Add to playlist',
    startRadio: 'Start radio',
    goToArtist: 'Go to artist',
    removeFav: 'Remove from favourites',
    addFav: 'Add to favourites',
    editData: 'Edit details',
    share: 'Share',
    saveToLibrary: 'Save to library',
    downloading: 'Downloading…',
    playOnDevice: 'Play on device',
    removeFromPlaylist: 'Remove from playlist',
    deleteFromLibrary: 'Delete from library',
    deleteTitle: 'Delete track',
    deleteMsg: '“{title}” will be deleted from the library and from disk.',
    deleteConfirm: 'Delete',
    notInterested: 'Not interested',
    feedbackSaved: 'We’ll make this less likely',
    feedbackFailed: 'Could not save that preference',
  },
  artistActions: {
    play: 'Play',
    shuffle: 'Shuffle play',
    goToArtist: 'Go to artist',
  },
  albumActions: {
    play: 'Play',
    shuffle: 'Shuffle play',
    goToAlbum: 'Go to album',
  },
  playlistActions: {
    play: 'Play',
    shuffle: 'Shuffle play',
    rename: 'Rename',
    renameTitle: 'Rename playlist',
    renameConfirm: 'Rename',
    duplicate: 'Duplicate',
    changeCover: 'Change cover',
    deleteList: 'Delete playlist',
    deleteTitle: 'Delete playlist',
    deleteMsg: '“{name}” will be deleted. The tracks will stay in your library.',
    deleteConfirm: 'Delete',
  },
  coverPicker: {
    header: 'Cover for “{name}”',
    none: 'No cover (automatic)',
    empty: 'The playlist has no tracks.',
  },
  deviceSheet: {
    sendingTo: 'Sending to {device}…',
    fallbackDevice: 'device',
    playingOn: 'Playing on device',
    failed: 'Could not control the device',
    title: 'Play on…',
    searching: 'Looking for devices…',
    emptyOthers: 'No other devices connected.',
    ariaOnline: 'Online',
    searchingShort: 'Looking…',
    empty: 'No devices connected.',
    selfSuffix: ' (this)',
    ariaPrev: 'Previous',
    ariaPause: 'Pause',
    ariaPlay: 'Play',
    ariaNext: 'Next',
  },
  pairDevice: {
    statusPending: 'Waiting for the device to scan…',
    statusClaimed: 'Pairing device…',
    statusCompleted: 'Device paired!',
    statusCancelled: 'Session cancelled',
    statusExpired: 'The code has expired',
    initError: 'Could not start pairing. Open the player on the host device.',
    linkedToast: 'Paired: {device}',
    fallbackDevice: 'device',
    codeCopied: 'Code copied',
    eyebrow: 'Pair device',
    pairTitle: 'Scan and connect',
    generating: 'Generating code…',
    qrAlt: 'Pairing QR code',
    codeLabel: 'Code',
    noteNoClaimUrl:
      'LAN access is unavailable: enter the code manually on the other device.',
    noteWithClaim: "If you can't scan, open the link and enter the code.",
    copy: 'Copy code',
    close: 'Close',
    cancel: 'Cancel',
    never: 'Never',
    revokeTitle: 'Revoke device',
    revokeMsg: 'Access for "{name}" will be revoked. It will need to be paired again.',
    revokeMsgFallback: 'this device',
    revokeConfirm: 'Revoke',
    revoked: 'Device revoked',
    revokeFailed: 'Could not revoke',
    loading: 'Loading…',
    empty: 'No paired devices yet.',
    pairedFallback: 'Paired device',
    phoneType: 'phone',
    lastUsed: 'used: {when}',
    revoke: 'Revoke',
    pairNew: 'Pair a new device',
  },
  metadataEditor: {
    title: 'Edit details',
    uploadCover: 'Upload cover',
    removeCover: 'Remove cover',
    fieldTitle: 'Title',
    fieldArtist: 'Artist',
    fieldAlbum: 'Album',
    fieldAlbumArtist: 'Album artist',
    cancel: 'Cancel',
    save: 'Save',
  },
  playlistPicker: {
    title: 'Add to playlist',
    new: 'New playlist',
    newPlaceholder: 'Playlist name',
    newConfirm: 'Create',
    empty: "You don't have any playlists yet.",
  },
  resumeBanner: {
    aria: 'Resume playback',
    bannerTitle: 'Resume playback?',
    autoTitle: 'Resume your DJ session?',
    autoSession: 'DJ session',
    fallbackTrack: 'A track',
    fromDevice: ' from {device}',
    fallbackDevice: 'another device',
    no: 'No',
    resume: 'Resume',
  },
  discoverySections: {
    moreLike: 'More like {artist}',
    coldStart: 'Start your library',
    madeForLibrary: 'Made for Your Library',
    rediscover: 'Rediscover',
    reasonTasteArtist:
      'Based on artists you play, save, favourite, or collect in playlists.',
    reasonRediscover: "Tracks in your library you haven't played in a while.",
    reasonPlaylist: 'Tracks from your playlist.',
    reasonColdStart:
      'Add music, import playlists, or search to teach Soundsible what to recommend.',
    reasonMadeForLibrary:
      'Local-first picks from favourites, playlists, and library structure.',
  },
  lyricsPanel: {
    noTrack: 'Play something to see its lyrics.',
    loading: 'Loading lyrics',
    error: 'Lyrics could not be loaded.',
    unavailable: 'The lyrics provider is not responding right now.',
    retry: 'Try again',
    instrumental: 'Instrumental track.',
    notFound: 'No lyrics found for this track.',
  },
  discoverNodes: {
    title: 'Recommendations',
    refresh: 'New recommendations',
    fromArtist: 'Because you listen to {artist}',
  },
  searchPanel: {
    ariaInLibrary: 'In your library',
    fallbackTitle: 'YouTube video',
    fallbackChannel: 'YouTube',
    noResolve: 'Could not resolve the track',
    inQueue: 'Already in queue',
    ariaSearch: 'Search music',
    panelTitle: 'Search',
    tabSearch: 'Search',
    tabDiscover: 'Discover',
    tabLyrics: 'Lyrics',
    radioTile: 'Song radio',
    radioTileSub: 'A mix seeded by what’s playing',
    surpriseTile: 'Surprise me',
    surpriseTileSub: 'Radio from a random pick of yours',
    similarSection: 'Similar to what’s playing',
    recentSearches: 'Recent searches',
    recentlySaved: 'Recently saved',
    discoverHint: 'Play and save music to teach Soundsible what to recommend.',
    moveLeft: 'Move panel left',
    moveRight: 'Move panel right',
    moveTitle: 'Move panel',
    closePanel: 'Close panel',
    placeholder: 'Search on Soundsible',
    clear: 'Clear search',
    hint: 'Search for songs, artists, or paste a YouTube link.',
    chipArtist: 'Artist',
    chipAlbum: 'Album',
    directSection: 'Detected video',
    songsSection: 'Songs',
    ytSection: 'YouTube results',
    searchError: 'The search could not be completed.',
    retry: 'Retry',
    noResults: 'No results.',
    ariaLoading: 'Loading',
    ariaQueued: 'In queue',
    ariaAddQueue: 'Add to queue',
    ariaMore: 'More options',
  },
  searchResultRow: {
    ariaRadio: 'Radio: more like this',
  },
  songRow: {
    ariaPlay: 'Play {title} by {artist}',
    ariaMore: 'More options',
  },
  trackList: {
    defaultEmpty: 'Nothing here.',
  },
  confirm: {
    cancel: 'Cancel',
    accept: 'OK',
  },
  prompt: {
    cancel: 'Cancel',
    save: 'Save',
  },
  libraryView: {
    unknownArtist: 'Unknown',
    noAlbum: 'No album',
  },
  format: {
    trackOne: 'track',
    trackOther: 'tracks',
  },
  toast: {
    trackFallback: 'Track',
    addingDownloads: 'Adding to downloads…',
    episodeInDownloads: 'Episode in downloads',
    downloadOperationFailed: 'Could not save the download change. Please try again.',
    downloadFailed: 'Could not download',
    addedToQueue: 'Added to queue',
    playNextConfirmed: 'Will play next',
    radioUnavailable: 'Radio is not available for podcasts',
    startingRadio: 'Starting radio…',
    noYtForRadio: 'Could not find the YouTube video to start the radio',
    noRelatedMix: 'No related songs found (seed: {ytId})',
    radioStarted: 'Radio started',
    radioFailed: 'Could not start the radio ({ytId})',
    radioFailedFallback: 'no ytId',
    trackDeleted: 'Track deleted',
    deleteFailed: 'Could not delete',
    dataUpdated: 'Details updated',
    updateFailed: 'Could not update',
    uploadingCover: 'Uploading cover…',
    coverUpdated: 'Cover updated',
    coverUploadFailed: 'Could not upload the cover',
    coverRemoved: 'Cover removed',
    coverRemoveFailed: 'Could not remove the cover',
    alreadyInLibrary: 'Already in your library',
    alreadyInDownloadsQueue: 'Already in the download queue',
    addedToDownloads: 'Added to downloads',
    addToDownloadsFailed: 'Could not add to downloads',
    playlistExists: 'A playlist with that name already exists',
    playlistCreated: 'Playlist created',
    playlistCreateFailed: 'Could not create the playlist',
    playlistDeleted: 'Playlist deleted',
    playlistDeleteFailed: 'Could not delete the playlist',
    playlistRenamed: 'Playlist renamed',
    playlistRenameFailed: 'Could not rename (duplicate name?)',
    playlistDuplicateSuffix: ' (copy)',
    playlistDuplicateSuffixN: ' (copy {n})',
    playlistDuplicated: 'Playlist duplicated',
    playlistDuplicateFailed: 'Could not duplicate',
    alreadyInPlaylist: 'Already in the playlist',
    addedToPlaylist: 'Added to "{name}"',
    addToPlaylistFailed: 'Could not add to the playlist',
    removedFromPlaylist: 'Removed from the playlist',
    removeFromPlaylistFailed: 'Could not remove from the playlist',
    reorderFailed: 'Could not reorder',
    playlistCoverUpdated: 'Cover updated',
    playlistCoverFailed: 'Could not change the cover',
    trackUnavailable: 'This track could not be played',
    trackUnavailableSkipping: 'Track unavailable — skipping',
    autoModeHandoffPaused: 'Previews unavailable right now — DJ will try again shortly',
    autoModeOpeningFailed: 'DJ could not find a playable opening in this source',
    audioNeedsGesture: 'Audio was interrupted — tap play to resume',
  },
  password: {
    new: 'New password',
    repeat: 'Repeat password',
    show: 'Show password',
    hide: 'Hide password',
    mismatch: 'Passwords do not match',
  },
  login: {
    title: 'Sign in',
    blurb: 'Sign in to reach your music.',
    username: 'Username',
    password: 'Password',
    signIn: 'Sign in',
    signingIn: 'Signing in…',
    failed: 'Wrong username or password',
  },
  account: {
    title: 'Account',
    signedInAs: 'Signed in as {name}',
    changeName: 'Change name',
    name: 'Your name',
    nameChanged: 'Name updated',
    nameFailed: 'Could not change the name',
    changeUsername: 'Change username',
    username: 'Username',
    usernameHint: 'Letters, digits, dot, dash or underscore. This is what you sign in with.',
    usernameChanged: 'Username updated',
    usernameFailed: 'That username is taken or not valid',
    changePassword: 'Change password',
    currentPassword: 'Current password',
    newPassword: 'New password',
    passwordChanged: 'Password updated',
    passwordFailed: 'Could not change the password',
    passwordFailedHint: 'Could not change it. Check that the current password is correct and that the two new entries match.',
    passwordHint: 'Type it twice to confirm.',
    manageUsers: 'People',
    signOut: 'Sign out',
    signOutConfirm: 'Sign out of Soundsible on this device?',
  },
  invite: {
    title: 'Set up your player',
    blurb: 'Pick a name and a password and you are in.',
    username: 'Username',
    password: 'Password',
    passwordHint: 'Pick whatever you like.',
    create: 'Create my account',
    creating: 'Creating…',
    failed: 'That name is taken, or the link is no longer valid.',
    invalidTitle: 'This link no longer works',
    invalidBlurb: 'Ask for a new one.',
  },
  users: {
    title: 'People',
    intro: 'Everyone here gets their own library, playlists, queue and history. Music files are shared, so nothing is downloaded twice.',
    adminOnly: 'Only an admin can manage accounts.',
    setYourPasswordFirst: 'Set a password on your own account before adding people — otherwise the server stays open to anyone on the network.',
    invite: 'Create invite link',
    inviteCopied: 'Link copied — send it to them',
    inviteReady: 'Link ready — copy it below',
    inviteFailed: 'Could not create the link',
    inviteHint: 'Single use, valid for 7 days. They choose their own password.',
    username: 'Username',
    displayName: 'Display name',
    password: 'Password',
    role: 'Role',
    roleAdmin: 'Admin',
    roleMember: 'Member',
    noPassword: 'No password',
    disabled: 'Disabled',
    add: 'Add person',
    addTitle: 'Add someone',
    created: 'Account created',
    createFailed: 'Could not create the account',
    setPassword: 'Set password',
    resetPasswordTitle: 'New password for {name}',
    resetPasswordMsg: 'They will be signed out everywhere.',
    passwordUpdated: 'Password updated',
    passwordFailed: 'Could not set the password',
    enable: 'Enable',
    disable: 'Disable',
    updateFailed: 'Could not update the account',
    deleteTitle: 'Delete {name}?',
    deleteMsg: 'Their library, playlists and history are removed. Shared music files stay on disk.',
    deleted: 'Account deleted',
    deleteFailed: 'Could not delete the account',
  },
  subsonic: {
    title: 'Other clients',
    note: 'Soundsible speaks OpenSubsonic, so apps like Symfonium, Amperfy, Feishin, DSub and Tempo can play your library — including offline and in the car.',
    server: 'Server address',
    username: 'Username',
    copyServer: 'Copy the address',
    password: 'Password',
    passwordNote: 'A password for these apps only, never your account password. Revoke it here and every app using it stops.',
    passwordShownOnce: 'Your new password',
    passwordShownOnceHint: 'Shown once. Leave this screen and it is gone — generate another one if you lose it.',
    copyPassword: 'Copy the password',
    generate: 'Generate a password',
    generateHint: 'Then add the server in your app with the address and username above',
    regenerate: 'Generate a new password',
    regenerateHint: 'Apps still using the old one will stop working',
    replaceTitle: 'Replace the current password?',
    replaceMessage: 'Any app already signed in with the old password stops working until you give it the new one.',
    revoke: 'Revoke access',
    revokeHint: 'No app can reach your library until you generate another password',
    revokeTitle: 'Revoke Subsonic access?',
    revokeMessage: 'Every app connected this way loses access immediately.',
    usage: 'Usage',
    lastUsed: 'Last used',
    lastClient: 'Last app',
    never: 'Never',
    copyFailed: 'Could not copy — select it and copy it by hand',
  },
  social: {
    copied: 'Copied to clipboard',
    copiedTextOnly: 'Copied — this one has no link to share',
    copyAction: 'Copy',
    copyManualTitle: 'Copy this link',
    copyManualTitleText: 'Copy this',
    copyManualHint: "Your browser wouldn't copy it on its own. Select it and copy it by hand.",
  },
  nowPlayingBrowser: {
    aria: 'Music browser',
    search: 'Search',
    searchGlobal: 'Search everywhere',
    globalSearch: 'Return to global search',
    searchLibrary: 'Search your library',
    listenNow: 'Listen now',
    libraryResults: 'Library results',
    searchEverywhere: 'Search “{query}” everywhere',
  },
};

export type Dict = typeof en;
