...
describe('trackCount', () => {
  ...
  // Artist, Playlists. Found by /qa on 2026-06-22.
  it('uses the singular for exactly one track', () => {
    expect(trackCount(1)).toBe('1 pista');
  });

  it('uses the plural for zero and for many', () => {
    expect(trackCount(0)).toBe('0 pistas');
    expect(trackCount(2)).toBe('2 pistas');
    expect(trackCount(125)).toBe('125 pistas');
  });
});
