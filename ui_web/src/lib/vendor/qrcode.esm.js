  ...
  return Pt || (Pt = 1, (function(r) {
    ...
    r.isValid = function(t) {
      ...
      for (let B = 0; B < n; B++) {
        ...
        for (let h = 0; h < n; h++) {
          let l = t.get(B, h);
          l === c ? a++ : (a >= 5 && (u += o.N1 + (a - 5)), c = l, a = 1), l = t.get(h, B), l === d ? s++ : (s >= 5 && (u += o.N1 + (s - 5)), d = l, s = 1);
        }
...
function de() {
  return Ut || (Ut = 1, (function(r) {
    const o = U(), i = Wt(), e = Ct(), t = F(), n = Zt(), u = 7973, a = o.getBCHDigit(u);
    function s(h, l, N) {
      for (let M = 1; M <= 40; M++)
    ...
    r.from = function(l, N) {
      ...
      typeof M > "u" && (M = t.BYTE);
      const S = o.getSymbolTotalCodewords(l), A = i.getTotalCodewordsCount(l, N), P = (S - A) * 8;
  ...
  return Ht || (Ht = 1, (function(r) {
    var o = {
      single_source_shortest_paths: function(i, e, t) {
        ...
        a.push(e, 0);
        for (var s, c, d, B, h, l, N, M, S; !a.empty(); ) {
          s = a.pop(), c = s.value, B = s.cost, h = i[c] || {};
          for (d in h)
            h.hasOwnProperty(d) && (l = h[d], N = B + l, M = u[d], S = typeof u[d] > "u", (S || M > N) && (u[d] = N, a.push(d, N), n[d] = c));
        }
...
function Ee() {
  return Jt || (Jt = 1, (function(r) {
    const o = F(), i = he(), e = me(), t = we(), n = Ce(), u = Xt(), a = U(), s = ye();
    function c(A) {
    ...
    function B(A) {
      const P = d(u.NUMERIC, o.NUMERIC, A), T = d(u.ALPHANUMERIC, o.ALPHANUMERIC, A);
      let p, b;
      return a.isKanjiModeEnabled() ? (p = d(u.BYTE, o.BYTE, A), b = d(u.KANJI, o.KANJI, A)) : (p = d(u.BYTE_KANJI, o.BYTE, A), b = []), P.concat(T, p, b).sort(function(y, C) {
    ...
    function M(A, P) {
      ...
      for (let g = 0; g < A.length; g++) {
        ...
        for (let f = 0; f < y.length; f++) {
          const R = y[f], m = "" + g + f;
          C.push(m), T[m] = { node: R, lastCount: 0 }, p[m] = {};
    ...
    function S(A, P) {
      ...
      if (T = o.from(P, p), T !== o.BYTE && T.bit < p.bit)
        throw new Error('"' + A + '" cannot be encoded with mode ' + o.toString(T) + `.
 Suggested mode is: ` + o.toString(p));
    ...
    r.fromArray = function(P) {
      ...
      const p = B(P, a.isKanjiModeEnabled()), b = N(p), g = M(b, T), y = s.find_path(g.map, "start", "end"), C = [];
...
function Be() {
  if (Ot) return W;
  Ot = 1;
  const r = U(), o = Ct(), i = oe(), e = ie(), t = ue(), n = se(), u = ae(), a = Wt(), s = fe(), c = de(), d = ge(), B = F(), h = Ee();
  function l(g, y) {
    const C = g.size, f = n.getPositions(y);
    for (let R = 0; R < f.length; R++) {
      const m = f[R][0], E = f[R][1];
      for (let w = -1; w <= 7; w++)
        if (!(m + w <= -1 || C <= m + w))
          for (let I = -1; I <= 7; I++)
            E + I <= -1 || C <= E + I || (w >= 0 && w <= 6 && (I === 0 || I === 6) || I >= 0 && I <= 6 && (w === 0 || w === 6) || w >= 2 && w <= 4 && I >= 2 && I <= 4 ? g.set(m + w, E + I, !0, !0) : g.set(m + w, E + I, !1, !0));
    }
  }
  function N(g) {
    const y = g.size;
  ...
  function M(g, y) {
    ...
    for (let f = 0; f < C.length; f++) {
      const R = C[f][0], m = C[f][1];
      for (let E = -2; E <= 2; E++)
        for (let w = -2; w <= 2; w++)
          E === -2 || E === 2 || w === -2 || w === 2 || E === 0 && w === 0 ? g.set(R + E, m + w, !0, !0) : g.set(R + E, m + w, !1, !0);
    }
  }
  function S(g, y) {
    ...
    let R, m, E;
    for (let w = 0; w < 18; w++)
      R = Math.floor(w / 3), m = w % 3 + C - 8 - 3, E = (f >> w & 1) === 1, g.set(R, m, E, !0), g.set(m, R, E, !0);
  }
  function A(g, y, C) {
    const f = g.size, R = d.getEncodedBits(y, C);
    let m, E;
    for (m = 0; m < 15; m++)
      E = (R >> m & 1) === 1, m < 6 ? g.set(m, 8, E, !0) : m < 8 ? g.set(m + 1, 8, E, !0) : g.set(f - 15 + m, 8, E, !0), m < 8 ? g.set(8, f - m - 1, E, !0) : m < 9 ? g.set(8, 15 - m - 1 + 1, E, !0) : g.set(8, 15 - m - 1, E, !0);
    g.set(f - 8, 8, 1, !0);
  }
  function P(g, y) {
    const C = g.size;
    let f = -1, R = C - 1, m = 7, E = 0;
    for (let w = C - 1; w > 0; w -= 2)
      for (w === 6 && w--; ; ) {
        for (let I = 0; I < 2; I++)
          if (!g.isReserved(R, w - I)) {
            let v = !1;
            E < y.length && (v = (y[E] >>> m & 1) === 1), g.set(R, w - I, v), m--, m === -1 && (E++, m = 7);
          }
        if (R += f, R < 0 || C <= R) {
          R -= f, f = -f;
          break;
  ...
  function T(g, y, C) {
    ...
    C.forEach(function(I) {
      f.put(I.mode.bit, 4), f.put(I.getLength(), B.getCharCountIndicator(I.mode, g)), I.write(f);
    });
    const R = r.getSymbolTotalCodewords(g), m = a.getTotalCodewordsCount(g, y), E = (R - m) * 8;
  ...
  function p(g, y, C) {
    const f = r.getSymbolTotalCodewords(y), R = a.getTotalCodewordsCount(y, C), m = f - R, E = a.getBlocksCount(y, C), w = f % E, I = E - w, v = Math.floor(f / E), V = Math.floor(m / E), $t = V + 1, yt = v - V, te = new s(yt);
    let O = 0;
    ...
    for (let k = 0; k < E; k++) {
      const G = k < I ? V : $t;
      H[k] = ee.slice(O, O + G), Et[k] = te.encode(H[k]), O += G, j = Math.max(j, G);
    }
    const Y = new Uint8Array(f);
    let Bt = 0, D, q;
    for (D = 0; D < j; D++)
      for (q = 0; q < E; q++)
        D < H[q].length && (Y[Bt++] = H[q][D]);
    for (D = 0; D < yt; D++)
      for (q = 0; q < E; q++)
        Y[Bt++] = Et[q][D];
    return Y;
...
`
    ...
    const E = T(y, C, R), w = r.getSymbolSize(y), I = new e(w);
    return l(I, y), N(I), M(I, y), A(I, C, 0), y >= 7 && S(I, y), P(I, E), isNaN(f) && (f = u.getBestMask(
      I,
      A.bind(null, I, C)
    )), u.applyMask(f, I), A(I, C, f), {
      modules: I,
      version: y,
  ...
  return W.create = function(y, C) {
    ...
    let f = o.M, R, m;
    return typeof C < "u" && (f = o.from(C.errorCorrectionLevel, o.M), R = c.from(C.version), m = u.from(C.maskPattern), C.toSJISFunc && r.setToSJISFunction(C.toSJISFunc)), b(y, R, f, m);
  }, W;
}
var ht = {}, mt = {}, jt;
function xt() {
  return jt || (jt = 1, (function(r) {
    ...
    r.getOptions = function(e) {
      ...
      for (let h = 0; h < c; h++)
        for (let l = 0; l < c; l++) {
          let N = (h * c + l) * 4, M = n.color.light;
          if (h >= d && l >= d && h < c - d && l < c - d) {
            const S = Math.floor((h - d) / s), A = Math.floor((l - d) / s);
            M = B[a[S * u + A] ? 1 : 0];
          }
...
function Re() {
  ...
  function e(t, n, u) {
    ...
    for (let B = 0; B < t.length; B++) {
      const h = Math.floor(B % n), l = Math.floor(B / n);
      !h && !c && (c = !0), t[B] ? (d++, B > 0 && h > 0 && t[B - 1] || (a += c ? i("M", h + u, 0.5 + l + u) : i("m", s, 0), s = 0, c = !1), h + 1 < n && t[B + 1] || (a += i("h", d), d = 0)) : s++;
    }
    return a;
  }
...
function Ae() {
  ...
  function t(n, u, a, s, c) {
    ...
    if (h) {
      ...
      if (B < 1)
        throw new Error("Too few arguments provided");
      return B === 1 ? (a = u, u = s = void 0) : B === 2 && !u.getContext && (s = a, a = u, u = void 0), new Promise(function(l, N) {
        try {
