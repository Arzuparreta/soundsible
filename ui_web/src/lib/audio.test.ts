...
describe('two-deck mixer', () => {
  it('keeps retired and staged sources excluded across mute and volume changes', async () => {
    vi.stubGlobal('AudioContext', FakeAudioContext);
    const diagnostics = await import('./playbackDiagnostics');
    diagnostics.startPlaybackDiagnostics({ variant: 'excluded', ios: '999', connection: 'bluetooth' });
    const module = await import('./audio');
    module.unlockAudio();
    await Promise.resolve();
    await module.audioService.load('/one', 1);
    const first = created[0], next = created[1];
    ...
    module.audioService.stage('/three', 1);
    module.audioService.setMuted(true);
    module.audioService.setVolume(0.4);
    module.audioService.setMuted(false);
    expect(first.muted).toBe(true);
    await module.audioService.takeStaged('/three', 1);
    expect(first.muted).toBe(false);
    expect(next.muted).toBe(true);
    ...
    const events = JSON.parse(diagnostics.playbackDiagnosticExport()).events;
    const excluded = events.findIndex((row: { event: string }) => row.event === 'deck.excluded');
    expect(events[excluded + 1].event).toBe('call.pause');
    expect(events[excluded + 1].media[0].muted).toBe(true);
  });

  it('delays normal retirement and preload but aborts safely on pause', async () => {
    vi.stubGlobal('AudioContext', FakeAudioContext);
    const diagnostics = await import('./playbackDiagnostics');
    diagnostics.startPlaybackDiagnostics({ variant: 'delayed', ios: '999', connection: 'carplay' });
    const module = await import('./audio');
    module.unlockAudio();
    await Promise.resolve();
    await module.audioService.load('/one', 1);
    ...
    expect(first.paused).toBe(true);
    expect(first.src).toBe('/three');
    await module.audioService.takeStaged('/three', 1);
    module.audioService.pause('ui');
    await module.audioService.resume('ui');
    await vi.advanceTimersByTimeAsync(9000);
    expect(first.paused).toBe(false);
    expect(next.paused).toBe(true);
  });

  it('releases a deferred DJ commitment on pause and permits a new paused capture', async () => {
    vi.stubGlobal('AudioContext', FakeAudioContext);
    const diagnostics = await import('./playbackDiagnostics');
    diagnostics.startPlaybackDiagnostics({ variant: 'delayed', ios: '999', connection: 'carplay' });
    const module = await import('./audio');
    ...
    await module.audioService.takeStaged('/two', 1);
    const handlers = callbacks();
    module.audioService.armTransition('/three', plan, handlers, { level: 1 });
    module.audioService.pause('ui');
    expect(handlers.onCancel).toHaveBeenCalledOnce();
    await vi.advanceTimersByTimeAsync(9000);
    expect(module.audioService.mixPhase()).toBe('idle');
    module.audioService.armTransition('/three', plan, callbacks(), { level: 1 });
    expect(module.audioService.mixPhase()).toBe('armed');
    expect(module.canConfigurePlaybackDiagnostics()).toBe(true);
  });

  it('excludes the outgoing DJ source on completion and preserves the incoming source', async () => {
    const diagnostics = await import('./playbackDiagnostics');
    diagnostics.startPlaybackDiagnostics({ variant: 'excluded', ios: '999', connection: 'bluetooth' });
