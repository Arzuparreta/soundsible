...
export function CollectionActions(props: {
  ...
  const use = async (purpose: 'reference' | 'request') => {
    if (busy()) return;
    ...
    try {
      if (props.items) {
        if (!await useCatalogCollection(props.items, props.title, purpose, before, isCurrent)) return;
      }
      ...
      if (isCurrent()) (props.onCompleted ?? placement.onCompleted)?.();
    } finally { setBusy(false); }
  };
  return <>
    <button type="button" disabled={busy() || !count()} aria-busy={busy()} onClick={() => props.auto ? void use(placement.referenceOnly ? 'reference' : 'request') : props.onPlay?.()}>
      {busy() ? t('collection.resolving') : t(props.auto ? (placement.referenceOnly ? 'musicExplorer.reference' : 'musicExplorer.requestAll') : 'playlistDetail.play')}
    </button>
    <Show when={props.auto && !placement.referenceOnly && !props.hideReferenceMenu}>
      <button type="button" disabled={busy() || !count()} aria-label={t('musicExplorer.reference')} title={t('musicExplorer.reference')} onClick={() => openActionMenu({ title: props.title, actions: [{ label: t('musicExplorer.reference'), onSelect: () => void use('reference') }] })}>•••</button>
    </Show>
  </>;
}
