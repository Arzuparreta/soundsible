...
def measure(client, headers, repeats):
    timings = []
    ...
    return response, {"status": response.status_code, "body_bytes": len(response.data),
...
def main():
    ...
    route.annotate_tracks = lambda tracks: None
    app = Flask(__name__)
    app.register_blueprint(route.library_bp)
    client = app.test_client()
    with user_context('benchmark'), patch('shared.artwork.artwork_store', return_value=SimpleNamespace(annotate=lambda tracks: None)):
        for count in args.sizes:
            ...
            full, full_stats = measure(client, {}, args.repeats)
            unchanged, unchanged_stats = measure(client, {'If-None-Match': full.headers['ETag']}, args.repeats)
